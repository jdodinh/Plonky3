//! BN254 field element with ligetron's root2 as the two-adic generator.
//!
//! This type is identical to [`Bn254`](super::Bn254) except it uses ligetron's
//! root2 as the primitive 2^28-th root of unity. Ligetron uses root2 for n=4k point NTTs.
...
/// The BN254 curve scalar field with ligetron's root2 as the two-adic generator.
///
/// This is the same field as [`Bn254`](super::Bn254), but with a different primitive
/// 2^28-th root of unity. Ligetron uses root2 for n=4k point NTTs.
#[derive(Copy, Clone, Default, Eq, PartialEq)]
#[must_use]
pub struct Bn254Root2 {
...
impl Bn254Root2 {
    ...
    ///
    /// Const version of `input.map(Bn254Root2::new)`.
    #[inline]
    ...
    /// Creates a new Bn254Root2 field element from an array of 4 u64's.
    ///
...
impl<'de> Deserialize<'de> for Bn254Root2 {
    #[inline]
...
impl Packable for Bn254Root2 {}

impl Hash for Bn254Root2 {
    #[inline]
...
impl PartialOrd for Bn254Root2 {
    #[inline]
...
impl PrimeCharacteristicRing for Bn254Root2 {
...
impl InjectiveMonomial<5> for Bn254Root2 {}

impl RawDataSerializable for Bn254Root2 {
...
quotient_map_small_int!(Bn254Root2, u128, [u8, u16, u32, u64]);
quotient_map_small_int!(Bn254Root2, i128, [i8, i16, i32, i64]);

impl QuotientMap<u128> for Bn254Root2 {
    #[inline]
...
impl QuotientMap<i128> for Bn254Root2 {
    #[inline]
...
impl PrimeField for Bn254Root2 {
    #[inline]
...
impl_add_assign!(Bn254Root2);
impl_sub_assign!(Bn254Root2);
impl_mul_methods!(Bn254Root2);
ring_sum!(Bn254Root2);
impl_div_methods!(Bn254Root2, Bn254Root2);

impl Distribution<Bn254Root2> for StandardUniform {
    #[inline]
    fn sample<R: Rng + ?Sized>(&self, rng: &mut R) -> Bn254Root2 {
        loop {
            let mut trial_element: [u8; 32] = rng.random();
            trial_element[31] &= (1_u8 << 6) - 1;
            let x = Bn254Root2::from_bytes_monty(&trial_element);
            if let Some(val) = x {
                return val;
            }
        }
...
/// TWO_ADIC_GENERATOR is the primitive 2^28-th root of unity used by ligetron (root2).
///
/// Ligetron uses root2 for n=4k point NTTs.
///
/// Standard form: 2037444462055058054189478067370099086220733342011840546702672064072905551290
/// Montgomery form: 19112272813437247237658335565886634816872559964956065028849099612849857838486
///
/// Verification in SageMath:
/// ```sage
/// P = 21888242871839275222246405745257275088548364400416034343698204186575808495617
/// R = 2^256
/// root2_standard = 2037444462055058054189478067370099086220733342011840546702672064072905551290
/// root2_montgomery = (root2_standard * R) % P
/// assert root2_montgomery == 19112272813437247237658335565886634816872559964956065028849099612849857838486
/// assert pow(root2_standard, 2^28, P) == 1
/// assert pow(root2_standard, 2^27, P) == P - 1
/// ```
...
impl TwoAdicField for Bn254Root2 {
...
mod tests {
    ...
    const ZERO: Bn254Root2 = Bn254Root2::ZERO;
    const ONE: Bn254Root2 = Bn254Root2::ONE;
    ...
    test_prime_field!(crate::Bn254Root2);
}
...
mod ligetron_compat_tests {
    ...
    fn test_two_adic_generator_is_root_of_unity() {
        let omega = Bn254Root2::two_adic_generator(28);
        ...
        assert_eq!(result, Bn254Root2::ONE, "omega^(2^28) should be 1");
    ...
    #[test]
    fn test_two_adic_generator_matches_ligetron_root2() {
        // Ligetron's root2 in standard form
        let ligetron_root2_standard = BigUint::parse_bytes(
            b"2037444462055058054189478067370099086220733342011840546702672064072905551290",
            10,
        )
        .unwrap();
        ...
        assert_eq!(
            our_standard, ligetron_root2_standard,
            "Our generator should match ligetron's root2"
        );
    }
}
